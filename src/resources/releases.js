...
export default class Release {
  constructor({ program, provider, http }) {
  ...
  async fetch(publicKey, withAccountData = false) {
    return await this.http.get(`/releases/${publicKey}`, undefined, withAccountData);
  };
  ...
  async fetchCollectors(publicKey, withCollection = false) {
    return await this.http.get(`/releases/${publicKey}/collectors${withCollection ? '?withCollection=true' : ''}`);
  };
  ...
  async fetchHubs(publicKey, withAccountData = false) {
    return await this.http.get(`/releases/${publicKey}/hubs`, undefined, withAccountData);
  };
  ...
  async fetchExchanges(publicKey, withAccountData = false, pagination) {
    return await this.http.get(`/releases/${publicKey}/exchanges`, pagination, withAccountData);
  };
  ...
  async fetchRevenueShareRecipients(publicKey, withAccountData = false) {
    return await this.http.get(`/releases/${publicKey}/revenueShareRecipients`, undefined, withAccountData);
  };
  ...
  async purchaseViaHub(releasePublicKey, hubPublicKey) {
    try {
      const { provider, endpoints } = this.client;
  ...
  async releaseInitViaHub(
    ...
    try {
      ...
      hubPublicKey = new anchor.web3.PublicKey(hubPublicKey);
      const hub = await this.program.account.hub.fetch(hubPublicKey);
      const paymentMint = new anchor.web3.PublicKey(isUsdc ? this.ids.mints.usdc : this.ids.mints.wsol);
      ...
      const metadataProgram = new anchor.web3.PublicKey(this.ids.programs.metaplex);
  ...
  async releaseInit(
    ...
    try {
      const { provider } = this.client;
      const paymentMint = new anchor.web3.PublicKey(isUsdc ? this.ids.mints.usdc : this.ids.mints.wsol);
      const publishingCreditMint = new anchor.web3.PublicKey(this.ids.mints.publishingCredit);
  ...
  async closeRelease(releasePublicKey) {
    try {
      const { provider } = this.client;
      const release = await this.program.account.release.fetch(new anchor.web3.PublicKey(releasePublicKey));
      const tx = await this.program.methods
        .releaseCloseEdition()
        .accounts({
          authority: provider.wallet.publicKey,
 ...
 */
  ...
  async addRoyaltyRecipient(recipientAddress, percentShare, releasePublicKey) {
    try {
      const { provider } = this.client;
      releasePublicKey = new anchor.web3.PublicKey(releasePublicKey);
        const release = await this.programprogram.account.release.fetch(releasePublicKey);
