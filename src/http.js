...
export default class Http {
  ...
  async fetchHubContentAndChildAccountData(publicKey, hubPublicKey, accountType) {
    ...
    if (accountType === 'post') {
      ...
      parsedChild = Formatter.parseHubReleaseAccountData(childAccount, publicKey);
    }
    ...
    const parsedHubContentAccount = Formatter.parseHubContentAccountData(hubContent, hubContentPublicKey);
  ...
  async getAccountData(url, response) {
    if (/^\/hubs\/((?!(\/)).)*$/.test(url)) {
      ...
      response.data.release.accountData = { release: Formatter.parseReleaseAccountData(release) };
      ...
      response.data.exchange.accountData = Formatter.parseExchangeAccountData(exchange);
    }
  ...
  async processMultipleHubCollaboratorAccountDataWithHub(data, hubPublicKey) {
    ...
    for await (let collaborator of collaborators) {
      ...
      const parsedCollaborator = Formatter.parseHubCollaboratorAccountData(collaborator, hubCollaboratorPublicKey);
  ...
  async processMultipleHubReleasesAccountDataWithHub(data, releasePublicKey) {
    ...
    let i = 0;
    for await (let hub of data) {
      const parsedHubRelease = Formatter.parseHubReleaseAccountData(hubReleases[i], hubReleasePublicKeys[i]);
      const parsedHubContent = Formatter.parseHubContentAccountData(hubContent[i], hubContentPublicKeys[i]);
      hub.accountData = {
        hub,
  ...
  async processHubReleaseAccountDataWithHub(releasePublicKey, hubPublicKey) {
    ...
    const parsedHubRelease = Formatter.parseHubReleaseAccountData(hubRelease, hubReleasePublicKey);
    const parsedHubContent = Formatter.parseHubContentAccountData(hubContent, hubContentPublicKey);
    const parsedRelease = Formatter.parseReleaseAccountData(release);
  ...
  async processMultipleHubContentAccountDataWithHub(data, hubPublicKey, accountType) {
    ...
    for await (let account of accounts) {
      ...
      if (accountType === 'release') {
        ...
        parsedChild = Formatter.parseHubReleaseAccountData(hubChildren[i], hubChildrenPublicKeys[i]);
      } else if (accountType === 'post') {
        parsedAccount = Formatter.parsePostAccountData(account);
        parsedChild = Formatter.parseHubPostAccountData(hubChildren[i], hubChildrenPublicKeys[i]);
      }
      const parsedHubContent = Formatter.parseHubContentAccountData(hubContent[i], publicKey);
